...
async function searchRestaurants(position, foodPreference) {
    try {
        ...
        const queryString = new URLSearchParams(queryParams).toString(); // URLSearchParams를 사용하여 쿼리 매개변수를 직렬화

        const response = await axios.get(`https://openapi.naver.com/v1/search/local.json?${queryString}`, {
            headers: {
                'X-NCP-APIGW-API-KEY-ID': 'ivo5law7se', // 네이버 API 키
                'X-NCP-APIGW-API-KEY': 'wuGt55gyiKqDzyQAunZJdNo5RDE7nyHWVIGVN8Q1' // 네이버 API 시크릿 키
            }
        });
